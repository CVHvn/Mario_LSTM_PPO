...
def get_args():
    ...
    parser.add_argument("--lstm_hidden_coef", type=float, default=0.01, help = 'lstm hidden coefficient. Use to penalty hidden state of lstm, make it change less')
...
def test(config):
    ...
    agent = Agent(world = config.world, stage = config.stage, action_type = config.action_type, envs = None, num_envs = config.num_envs,
              ...
              target_kl = config.target_kl, gae_lambda = config.gae_lambda,
              lstm_hidden_coef = config.lstm_hidden_coef, device = "cuda" if torch.cuda.is_available() else "cpu")
